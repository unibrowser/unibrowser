...
from scraping.Constants import SPORTS_LINKS, SPORT_ID, AWAY, HOME, LOCATION, TIME, DATE, PROGRESS, DETAILS, TICKETS, SPORT, AIRING, TEAM_LOGO, NAME, NAME_ABBR, SCORE, ALT_TILE 
...
#For Case 3
    if teamDetails is not None:
        teamDetailsTitle = teamDetails.find("div", {"class":"team-detail title"})
        if teamDetailsTitle is not None:
            ...
            if awayScore is not None:
                ...
                futureGameTime = teamDetails.find("span", {"class":"DateTimeDisplay"})
            ...
            print("awayAbbrTitle : ", awayAbbrTitle, " homeAbbrTitle : ", homeAbbrTitle)
            data[AWAY] = away
            data[HOME] = home 

def getMatchPlayers(teamDetails, away, home):
    awayTeamLogo, awayTeamTitle = getLogoDetails(teamDetails.find("span", {"class":"ScheduleEventDetail away"}))
    print("awayTeamTitle : ", awayTeamTitle, " awayTeamLogo: ", awayTeamLogo)
    away[TEAM_LOGO] = awayTeamLogo
    away[NAME] = awayTeamTitle
    homeTeamLogo, homeTeamTitle = getLogoDetails(teamDetails.find("span", {"class":"ScheduleEventDetail home"}))
...
def getMatchMeta(div, data):
    ...
    eventDetails = div.find("a", {"class":"Link events-link-details Link-events-link-details"})
    ...
    eventTickts = div.find("a", {"class":"Link events-link-tickets Link-events-link-tickets"})
...
def readSportsList():
    with open(SPORTS_LINKS, 'r') as myfile:
